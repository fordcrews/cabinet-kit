...
test("Klondike cascades all safe foundation cards until none remain", () => {
  const session = E.createKlondikeSession(def, seedRng(21));
  session.tableau = [
    [face("A", "♥", true)],
    [face("2", "♥", true)],
    [face("3", "♥", true)],
  ...
  session.score = 0;
  // 3♥ starts unsafe (♣ helper only to A); autoplaying 2♣ unlocks it after A♥/2♥.
  assert.equal(E.patienceAutoSafe(card("3", "♥"), session.foundations), false);
  const n = E.autoPlayPatience(session);
  assert.ok(n >= 4);
  assert.equal(session.foundations["♥"], 3);
  assert.equal(session.foundations["♣"], 2);
  assert.equal(session.tableau[0].length, 0);
  assert.equal(session.tableau[1].length, 0);
  assert.equal(session.tableau[2].length, 0);
  assert.equal(session.tableau[3].length, 0);
});

test("Klondike cascades waste then newly flipped tableau Ace", () => {
  const session = E.createKlondikeSession(def, seedRng(22));
  session.tableau = [
    [face("A", "♠", false), face("2", "♥", true)],
    [],
    [],
    [],
    [],
...
test("FreeCell cascades every legal foundation move (not Klondike-safe filter)", () => {
  const session = E.createFreeCellSession(freecellDef, seedRng(23));
  session.tableau = [
    [face("3", "♥")],
    [face("4", "♥")],
    [],
  ...
  session.cells = [face("A", "♥"), face("2", "♥"), null, null];
  session.foundations = { "♠": 0, "♥": 0, "♦": 0, "♣": 0 };
  session.foundationTops = { "♠": null, "♥": null, "♦": null, "♣": null };
  session.score = 0;
  // Mid-ranks are legal but would fail Klondike patienceAutoSafe until helpers exist.
  assert.equal(E.patienceAutoSafe(card("3", "♥"), { "♠": 0, "♥": 2, "♦": 0, "♣": 0 }), false);
  const n = E.autoPlayPatience(session);
  assert.equal(n, 4);
  assert.equal(session.foundations["♥"], 4);
  assert.equal(session.cells[0], null);
  assert.equal(session.cells[1], null);
  assert.equal(session.tableau[0].length, 0);
  assert.equal(session.tableau[1].length, 0);
  assert.equal(session.score, 40);
});

test("FreeCell still leaves illegal foundation cards", () => {
  const session = E.createFreeCellSession(freecellDef, seedRng(24));
  session.tableau = [[face("5", "♠")], [], [], [], [], [], [], []];
  session.cells = [null, null, null, null];
  session.foundations = { "♠": 3, "♥": 0, "♦": 0, "♣": 0 };
  session.foundationTops = {
    "♠": { rank: "3", suit: "♠" },
    "♥": null,
    "♦": null,
