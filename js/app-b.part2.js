  ...
  if (ui.undo) {
    ui.undo.addEventListener("click", function () {
      if (session && isMahjong() && (session.status === "playing" || session.status === "stuck" || session.status === "won")) {
  ...
  if (ui.hint) {
    ui.hint.addEventListener("click", function () {
      if (!session || !isMahjong() || session.status !== "playing") return;
      E.hintMahjong(session);
      renderGame();
    });
  }
  if (ui.mahjongBoard) {
    ui.mahjongBoard.addEventListener("click", function (ev) {
      if (!session || !isMahjong() || session.status !== "playing") return;
      const btn = ev.target.closest("[data-mj]");
      if (!btn) return;
      E.tapMahjong(session, Number(btn.getAttribute("data-mj")));
      renderGame();
    });
  }
